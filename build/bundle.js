...
canvas.addEventListener('click', function (event) {
    ...
    particles.push(new _Particle2.default(ctx, event.x, event.y, Math.random() * 3 + 1, Math.random() * 3 + 1, radius, Math.random() * 2 + 1));
});

for (var i = 0; i < 30; i++) {
    var radius = Math.random() * 5 + 5;
    var x = _Particle2.default.randomCoord(canvas.width, radius);
    var y = _Particle2.default.randomCoord(canvas.height, radius);
    particles.push(new _Particle2.default(ctx, x, y, Math.random() * 2, Math.random() * 2, radius, Math.random() * 2 + 1));
}
...
function animate() {
    ...
    particles.forEach(function (particle, idx) {
        var remainingParticles = [].concat(particles).splice(idx + 1);
        remainingParticles.forEach(function (particleB) {
            _Particle2.default.detectCollisionAndDistance(particle, particleB);
        });
        particle.update();
    });
}
...
var Particle = function () {
    _createClass(Particle, null, [{
        ...
        key: 'randomCoord',
        value: function randomCoord(maxValue, particleRadius) {
        ...
        key: 'handleCollistion',
        value: function handleCollistion(angle, m1, m2, velocityA, velocityB) {
            var u1 = Particle.rotate(velocityA, angle);
            var u2 = Particle.rotate(velocityB, angle);

            var v1 = { x: u1.x * (m1 - m2) / (m1 + m2) + u2.x * 2 * m2 / (m1 + m2), y: u1.y };
            var v2 = { x: u2.x * (m1 - m2) / (m1 + m2) + u1.x * 2 * m2 / (m1 + m2), y: u2.y };

            var vFinal1 = Particle.rotate(v1, -angle);
            var vFinal2 = Particle.rotate(v2, -angle);
            var deltaRadius = Math.random() - 0.5;
            var newDirection = {
                velocityA: {
        ...
        key: 'detectCollisionAndDistance',
        value: function detectCollisionAndDistance(particleA, particleB) {
            var xDist = Math.abs(particleA.x - particleB.x);
            var yDist = Math.abs(particleA.y - particleB.y);

            var distance = Math.hypot(xDist, yDist) - particleA.radius - particleB.radius;
            if (distance < 1) {
                var velocityA = { x: particleA.dx * particleA.xTurn, y: particleA.dy * particleA.yTurn };
                var velocityB = { x: particleB.dx * particleB.xTurn, y: particleB.dy * particleB.yTurn };
                var newDirection = Particle.handleCollistion(-Math.atan2(particleB.y - particleA.y, particleB.x - particleA.x), particleA.radius, particleB.radius, velocityA, velocityB);
            ...
            if (distance < 250) {
                var ctx = particleA.ctx;
                var gradient = ctx.createLinearGradient(particleA.x, particleA.y, particleB.x, particleB.y);
    ...
    function Particle(ctx, x, y, dx, dy, radius, lineWidth) {
    ...
    _createClass(Particle, [{
        ...
        key: 'checkForFrameHit',
        value: function checkForFrameHit() {
